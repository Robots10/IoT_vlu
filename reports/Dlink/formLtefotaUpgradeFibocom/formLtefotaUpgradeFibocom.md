# D-Link Router DIR-825m -  Buffer Overflow in /boafrm/formLtefotaUpgradeFibocom
...
During a security review of the router's firmware, a critical vulnerability was identified in the `/boafrm/formLtefotaUpgradeFibocom` endpoint.

The vulnerability resides within the `sub_41802C` function, which is executed when triggering an LTE module firmware upgrade. The function extracts the user-controlled `fota_url` parameter from the HTTP POST request and processes it. The function performs a loose validation using `strstr` to verify whether the protocol prefix (`"http://"`, `"https://"`, or `"ftp://"`) exists inside the input.
...
- **Vulnerability Location**: `/boafrm/formLtefotaUpgradeFibocom`
...
In `sub_41802C`, the parameter `fota_url` is fetched and stored in `v4`:
...
```c
sprintf(v6, "at-mngr 'AT+GTSET=\"CALLBREAK\",1'; /var/upgrade_fibocom %s&", v4);
system(v6);
```
...
The program uses `sprintf` to write the formatted command into `v6`. Since `sprintf` does not perform bounds checking, providing a `fota_url` parameter larger than 452 bytes will overflow the bounds of `v6`, overwriting adjacent stack memory, stack frames, and the saved return address (`$ra` in MIPS/ARM).
...
Referer: http://192.168.0.1/fota_fibocom.htm
